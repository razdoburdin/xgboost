...
void GHistBuilderTest(float sparsity, bool force_atomic_use) {
  ...
  event_in = InitHist(qu, &hist, hist.Size(), event_in);
  event_in = InitHist(qu, &hist_buffer, hist_buffer.Size(), event_in);

  auto event = builder.BuildHist(gpair, row_set_collection[0], gmat_sycl, &hist,
                                 sparsity < eps , &hist_buffer, event_in, {}, force_atomic_use);
...
void GHistSubtractionTest() {
  ...
  ::sycl::event event0 = qu->memcpy(hist1.Data(), hist1_host.data(),
  ...
  ::sycl::event event1 = qu->memcpy(hist2.Data(), hist2_host.data(),
                                    2 * n_bins * sizeof(GradientSumT));

  std::vector<GradientSumT> hist3_host(2 * n_bins);
  GHistType hist3(qu, 2 * n_bins);
  ::sycl::event event = SubtractionHist(qu, &hist3, hist1, hist2, n_bins, {event0, event1});
