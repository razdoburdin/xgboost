...
// =======
  auto hist_builder = common::GHistBuilder<GradientSumT>();
...
// >>>>>>> a20b4d1a... partition optimizations
  ...
  std::vector<std::vector<GradientSumT>> hists(1);
  ...
  GradientSumT* hist_data = reinterpret_cast<GradientSumT*>(hist[0].data());
  // reduce hists
