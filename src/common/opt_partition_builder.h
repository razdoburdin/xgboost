...
class ThreadsManager {
 public:
  ...
  class CyclicBuffer {
    ...
    public:
    CyclicBuffer(StorageT* storage_ptr) : storage_ptr_(storage_ptr) {}

    size_t Id() const {
      return counter_ % storage_ptr_->size();
    }
  ...
  ThreadsManager() : threads(), nodes(), threads_cbuffer(&threads) {}
  ...
  CyclicBuffer<std::vector<ThreadInfo>> threads_cbuffer;
};
...
class OptPartitionBuilder {
 public:
    ...
    switch (column_matrix.GetTypeSize()) {
      case common::kUint8BinsTypeSize:
        CommonPartition<BinTypeMap<kUint8BinsTypeSize>::Type, is_loss_guided, all_dense, any_cat>(
                           column_matrix, std::forward<Predicate>(pred), args...);
        break;
      case common::kUint16BinsTypeSize:
  ...
  void CommonPartition(const ColumnMatrix& column_matrix, Predicate&& pred,
                       size_t tid, const size_t row_indices_begin, 
    ...
    const BinIdxType* numa = column_matrix.template GetIndexData<BinIdxType>();
    ...
    if (is_loss_guided) {
      ...
      for (uint32_t i = 0; i < n_threads; ++i) {
        while (curr_thread_size != 0) {
          auto& thread_info = tm.threads_cbuffer.GetThread();
          uint32_t node_id = tm.threads_cbuffer.NCycles();
          uint32_t tid = tm.threads_cbuffer.Id();
          const uint32_t curr_thread_node_size = thread_info.nodes_count[node_id];
          if (curr_thread_node_size == 0) {
            tm.threads_cbuffer.Next();
          } else if (curr_thread_node_size > 0 && curr_thread_node_size <= curr_thread_size) {
            const uint32_t begin = nodes_count[tid][node_id].first;
            CHECK_EQ(nodes_count[tid][node_id].first + curr_thread_node_size,
                     nodes_count[tid][node_id].second);
            tm.threads[i].addr.push_back({
              threads_rows_nodes_wise[tid].data(), begin,
            ...
            nodes_count[tid][node_id].first += curr_thread_size;
            curr_thread_size = 0;
          }
        }
      ...
      for (uint32_t i = 0; i < n_threads; ++i) {
        ...
        while (curr_thread_size != 0) {
          if (curr_vec_rowssize > curr_thread_size) {
            ...
            tm.threads[i].addr.push_back({vec_rows[curr_vec_rowsid].data(),
                                      1 + vec_rows[curr_vec_rowsid][0] - curr_vec_rowssize,
