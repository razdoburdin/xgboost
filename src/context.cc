...
- sycl:<cpu, gpu>:<device ordinal> # e.g. sycl:gpu:0
  ...
  if (!std::regex_match(input, pattern)) {
    LOG(FATAL) << "device doesn't match regex pattern";
    fatal();
  }
  ...
  if (split_it == s_device.cend()) {
    ...
    if (s_device == DeviceSym::CPU()) {
      ...
      LOG(FATAL) << "device doesn't match switch statement";
      fatal();
    }
  } else {
    ...
    if (s_ordinal.empty()) {
      LOG(FATAL) << "Problem" << s_device.cend() - split_it;
      fatal();
    }
    auto opt_id = ParseInt(s_ordinal);
    if (!opt_id.has_value()) {
      LOG(FATAL) << "Problem2" << s_device.cend() - split_it;
      fatal();
    }
  ...
  if (device.ordinal < Context::kCpuId) {
    LOG(FATAL) << "Wrong device.ordinal" << device.ordinal;
    fatal();
  }
  if (device.IsCUDA()) {
