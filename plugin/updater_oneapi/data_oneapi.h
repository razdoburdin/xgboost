...
//   Bug. Copy constructor doesn't copy data.
...
//   }
  ...
  // T Get(sycl::queue qu, size_t idx, std::vector<sycl::event>* events_ptr) const {
  //   T val;
  //   auto event = qu.memcpy(&val, data_.get() + idx, sizeof(T), *events_ptr);
  //   events_ptr->emplace_back(event);
  //   return val;
  // }

  // T Get(sycl::queue& qu, size_t idx) const {
  //   T val;
  //   last_event_ = qu.memcpy(&val, data_.get() + idx, sizeof(T));
  //   return val;
  // }
...
private:
  ...
  // mutable sycl::event last_event_;
};
